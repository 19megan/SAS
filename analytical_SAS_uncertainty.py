# This script will find uncertainty bounds on SAS functions based on an analytical
# approach by feeding a discretized concentration corrupted with noise 
# once in a sequential timestep over a long period of time.
...
# This script needs a given SAS function form, J, and Q, and noise corruption on tracers
# Date: 12/09/2025
...
date = pd.date_range(start='2018-01-01 00:00:00', end='2018-12-31 23:00:00', freq='h') #2018-01-01 - 2020-12-31
...
# For daily rainfall use dt=24, for hourly rainfall use dt=1
dt=1
k=2400 # hours (100 days)
Q,Sm = simulate_discharge(df['J (mm/hr)'].values, dt=dt, K=k, S0=df['J (mm/hr)'].mean()*k)
df['Q (mm/hr)'] = Q
...
mu, sigma = 0, 0.01 # mean and std of noise
noise = np.random.normal(mu, sigma, size=len(df))
# noise = 0 # for no noise case
dense_C = sparse.diags(np.ones(len(df)) + noise, offsets=0, format='csc')

# to add noise to diagonal offsets:
# offset_noise = np.random.normal(0, 0.02, size=len(df)-1)
# dense_C = sparse.diags([np.ones(len(df)) + noise, offset_noise, offset_noise], offsets=[0,1,-1], format='csc')

C_df = pd.DataFrame.sparse.from_spmatrix(dense_C, index=df.index, columns=tracers)
...
if True:
    ...
    plt.plot(ST[f'obs time {i}'], Pq[f'obs time {i}'], label='Analystical SAS', marker='.') #if i cumsum over ages
plt.xlabel('$S_T$ (mm)')
...
plt.plot(ST_true[f'obs time {i}'], Pq_true[f'obs time {i}'], label='Zero noise uniform SAS', zorder=1, marker='.') #uniform SAS line
...
#%%
# -------------------find residual between noise and true SAS
# note: need to run without noise first and save Pq_true and ST_true

# S_max = df['Storage (dt-mean) (mm)'].iloc[-1]
# ST_true = np.linspace(0, S_max, len(ST))
# Pq_true = ST_true / S_max
residual_pq = Pq[f'obs time {i}'] - Pq_true[f'obs time {i}']
residual_st = ST[f'obs time {i}'] - ST_true[f'obs time {i}']
plt.figure(figsize=(10,4))
plt.scatter(residual_st, Pq[f'obs time {i}'], c=np.arange(len(residual_st)), cmap='viridis', label='Residual SAS', marker='.')
plt.plot(residual_st, Pq[f'obs time {i}'], marker='.', zorder=-1, alpha=.3) #line to show trend
cbar = plt.colorbar()
cbar.set_label('Tracer Age Index')
plt.axhline(0, color='black', linestyle='--')
plt.axvline(0, color='black', linestyle='--')
plt.xlabel('Residual $S_T$ (mm)')
plt.ylabel('Cumulative TTD $P_Q$')
plt.title('Residual with added noise ~N({mu},{sigma}^2) and offset noise ~N(0,0.2^2)'.format(mu=mu, sigma=sigma))
plt.show()
